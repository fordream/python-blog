...
def create_engine(user, passwd, database, host='localhost', port='8889', **kw):
    ...
    if engine is not None:
        DBError('Engine has already initialized!')
    params = dict(user=user, database=database, passwd=passwd, host=host, port=port)
    defaults = dict(use_unicode=True, charset='utf8', collation='utf8_general_ci', autocommit=False)
    settled = dict(buffered=True)
    for k, v in defaults.items():
        params[k] = kw.pop(k, v)
    params.update(defaults)
    params.update(settled)
...
if __name__ == '__main__':
    ...
    create_engine('root', 'root', 'test', 'localhost', '8889')
